...
class GameState:
    ...
    def generateSuccessor(self, agentIndex, action):
        ...
        if agentIndex == 0:
            ...
            state.data.ghostScore += -TIME_PENALTY/2
        ...
        #if agentIndex == 1:
         #   GhostRules.decrementGlobalTimer(state)
        # Book keeping
    ...
    def getGhostScore(self):
        return float(self.data.ghostScore)
...
class ClassicGameRules:
    ...
    def decrementGlobalTimer(self,state):
        global max_time_to_run
        timer = max_time_to_run
        if timer == 1:
    ...
    def win(self, state, game):
        if not self.quiet:
            #print(("Pacman emerges victorious! Score: %d" % state.data.score,state.data.ghostScore))
            print(("Ghost emerges victorious! Score: %d" % state.data.ghostScore))
        game.gameOver = True

    def lose(self, state, game):
        if not self.quiet:
            # print(("Pacman died! Score: %d" % state.data.score,state.data.ghostScore))
            print(("Ghost could not capture Pacman! Score: %d" % state.data.ghostScore))
        game.gameOver = True
...
def readCommand(argv,pacmantrain=True,pacmanTypestr="Smart",pacmanInstance=None):
    ...
    parser.add_option('-p', '--pacman', dest='pacman',
                      ...
                      metavar='TYPE', default='PacmanBaseAgent')
    ...
    for k in range(0,options.numGhosts):
        if pacmantrain == True:
    ...
    #print(ghostType[0],ghostType[1],options.numGhosts)
...
def loadAgent(pacman, nographics):
    ...
    for moduleDir in pythonPathDirs:
        ...
        moduleNames = [f for f in os.listdir(
            moduleDir) if f.endswith('gents.py') or f.endswith('gents.pyc')]
